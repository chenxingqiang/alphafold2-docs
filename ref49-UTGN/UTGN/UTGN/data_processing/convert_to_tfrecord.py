...
Command-line arguments:
    input file
    output file
    number of entries in evo profiles
"""
...
def dict_to_tfrecord(dict_):
    ...
    feature_lists_dict.update({
        'primary':
        _feature_list([_int64_feature([aa]) for aa in dict_['primary']])
    })
    ...
    if 'evolutionary' in dict_:
        feature_lists_dict.update({
            'evolutionary':
            _feature_list([
                _float_feature(list(step))
                for step in zip(*dict_['evolutionary'])
            ])
        })
    ...
    if 'secondary' in dict_:
        feature_lists_dict.update({
            'secondary':
            _feature_list(
                [_int64_feature([dssp]) for dssp in dict_['secondary']])
        })
    ...
    if 'tertiary' in dict_:
        feature_lists_dict.update({
            'tertiary':
            _feature_list([
                _float_feature(list(coord))
                for coord in zip(*dict_['tertiary'])
            ])
        })
    ...
    if 'mask' in dict_:
        feature_lists_dict.update({
            'mask':
            _feature_list([_float_feature([step]) for step in dict_['mask']])
        })

    record = _sequence_example(
...
if __name__ == '__main__':
    ...
    num_evo_entries = int(sys.argv[3]) if len(sys.argv) == 4 else 20
